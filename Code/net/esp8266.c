...
#define MQTT_CLIENT_ID   "mqtt_stm32|securemode=2\\,signmethod=hmacsha1\\,timestamp=1687860985935|"   
#define MQTT_USER_NAME   "mqtt_stm32&a1THAR4Jl2A"
#define MQTT_PASSWD      "A1474635BC21C718502826E602E09B6143A68450"
#define BROKER_ASDDRESS  "a1THAR4Jl2A.iot-as-mqtt.cn-shanghai.aliyuncs.com"
#define SUB_TOPIC        "/sys/a1THAR4Jl2A/mqtt_stm32/thing/service/property/set"
#define PUB_TOPIC        "/sys/a1THAR4Jl2A/mqtt_stm32/thing/event/property/post"
...
uint8_t parse_json_msg(uint8_t *json_msg,uint8_t json_len)
{
  ...
  JSONStatus_t result;
  signed char query[] = "params.led";
  size_t queryLength = sizeof( query ) - 1;
  char * value;
  size_t valueLength;
  result = JSON_Validate((const signed char *)json_msg, json_len);
  if( result == JSONSuccess)
  {
    result = JSON_Search((signed char *)json_msg, json_len, query, queryLength,&value, &valueLength );
    if( result == JSONSuccess)
    {
